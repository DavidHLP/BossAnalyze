...
export function useFollowRoll() {
  ...
  let destroyHandler: null | (() => void) = null
  let isScrollingFromCode = false // 防止循环触发的标志
  let isScrollingFromPreview = false // 防止循环触发的标志
  ...
  function scrollHandler() {
    ...
    setTimeout(() => {
      ...
      // 获取预览滚动容器
      const previewViewport = document.querySelector('.preview-viewport, .markdown-render') as HTMLElement

      if (!editorScroller || !previewViewport) {
        console.warn('跟随滚动：未找到编辑器或预览容器，将重试获取')
        return null
      }

      setupScrollSync(editorScroller, previewViewport)
    }, 100)

    return null
  }

  function setupScrollSync(editorScroller: HTMLElement, previewViewport: HTMLElement) {

    // 编辑器滚动回调 - 同步到预览
    const editorScrollCallback = useThrottleFn(() => {
      if (!followRoll.value || isScrollingFromPreview) return
      ...
      // 同步到预览
      const targetScrollTop = (previewViewport.scrollHeight - previewViewport.clientHeight) * scrollRatio
    ...
    // 预览滚动回调 - 同步到编辑器
    const previewScrollCallback = useThrottleFn(() => {
      if (!followRoll.value || isScrollingFromCode) return

      isScrollingFromPreview = true

      // 计算预览滚动比例
      const scrollRatio = previewViewport.scrollTop / (previewViewport.scrollHeight - previewViewport.clientHeight)

      // 同步到编辑器
      const targetScrollTop = (editorScroller.scrollHeight - editorScroller.clientHeight) * scrollRatio
    ...
    // 添加滚动监听
    editorScroller.addEventListener('scroll', editorScrollCallback)
    previewViewport.addEventListener('scroll', previewScrollCallback)

    // 保存清理函数到全局变量
    destroyHandler = () => {
      editorScroller?.removeEventListener('scroll', editorScrollCallback)
      previewViewport?.removeEventListener('scroll', previewScrollCallback)
    }
  }

  function setFollowRoll(val: string | number | boolean = followRoll.value) {
    // 设置状态
    followRoll.value = Boolean(val)
...
export function splitPage(renderCV: HTMLElement) {
  ...
  // 检查 reRender 元素是否存在
  if (!reRender) {
    console.warn('splitPage: 未找到 .re-render 元素，跳过分页处理')
    return
  }

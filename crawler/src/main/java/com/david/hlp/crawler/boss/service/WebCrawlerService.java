...
public class WebCrawlerService {
    ...
    private static final String[] exceptionHtml = {
        "正在加载中...",
        "正在加载中",
        "网站访客身份验证 - BOSS直聘",
    ...
    public HtmlData getHtmlContent(HtmlData htmlData) {
        ...
        try {
            ...
            // 检查HTML内容是否包含异常信息
            if (checkHtmlException(html)) {
                log.warn("获取的页面内容异常: {}", targetUrl);
     ...
     * @return 如果包含异常信息返回true，否则返回false
     */
    private boolean checkHtmlException(String htmlContent) {
        if (htmlContent == null || htmlContent.isEmpty()) {
            return false;
        }
        
        for (String exception : exceptionHtml) {
            if (htmlContent.contains(exception)) {
                log.warn("检测到HTML内容包含异常信息: {}", exception);
                return true;
            }
        }
